...
# NOTE ON THE POLICY DIMENSION
# ----------------------------
# COMPLIANT now requires all four of POLICY_STATUS=FOUND, POLICY_ASSESSMENT=COMPLIANT,
# EVIDENCE_STATUS=FOUND, EVIDENCE_ASSESSMENT=COMPLIANT (.agents/AGENTS.md), with no
# NOT_REQUIRED/NOT_APPLICABLE escape hatch -- this fixture used to set exactly those two
# removed values. The fixtures below supply no policy artifact, so post_process
# deterministically resolves policy to NOT_FOUND and the FINAL status is NON_COMPLIANT
# for every case here, whatever the evidence says.
#
# That makes the final status useless for the questions THIS file asks, which are all
# about grounding, negation polarity and contradiction on the EVIDENCE side. So the
# "must still pass" tests assert the evidence-level outcome (the thing being tested)
# rather than the final verdict (which the absent policy now decides). Asserting
# NON_COMPLIANT instead would make them vacuous -- they would keep passing even if the
# negation defects they exist to catch came back.
_BASE = {
    "confidence": 9,
...
def test_unrelated_negation_does_not_sink_a_real_pass():
    ...
    # Evidence-level, not final status -- see the note on _BASE above.
    assert out["evidence_assessment"] == "COMPLIANT"
    assert out.get("contradiction_suspected") is not True
...
def test_excel_scoped_evidence_only_control_still_needs_a_policy():
    """Excel scoping does NOT waive the policy requirement -- decided deliberately.

    5.33 is clause 5, so derive_policy_required() defaults it to needing a policy, and
    the auditor's checklist here scoped it to a single screenshot that cannot contain
    one. A waiver used to fire for exactly this case so the control was not unpassable.

    That waiver has been removed: the compliance rule is unconditional
    (.agents/AGENTS.md) -- COMPLIANT requires policy FOUND + COMPLIANT and evidence
    FOUND + COMPLIANT, no exceptions -- so this now reports a real policy gap instead.
    The archival evidence itself is still recognised as good; only the verdict changes.
    """
    out = _run({**_BASE,
    ...
    assert out["status"] == "NON_COMPLIANT"
    assert out["policy_status"] == "NOT_FOUND"
    # The flag is gone entirely -- nothing sets it any more.
    assert not out.get("policy_requirement_waived")


def test_unscoped_clause5_control_still_requires_policy():
    """Without Excel scoping the clause default stands. Kept as the unscoped
    counterpart to the test above: both scoped and unscoped now require a policy."""
    out = _run({**_BASE,
                "control_id": "5.33 Protection of Records",
                "question": "5.33 Protection of Records - Whether log archival is done?",
                "status": "COMPLIANT",
...
def test_scoped_control_explicitly_asking_for_a_policy_still_requires_one():
    """A question that literally asks for a policy document demands one, scoped or
    not -- the strongest form of the rule, and unaffected by the waiver's removal."""
    out = _run({**_BASE,
