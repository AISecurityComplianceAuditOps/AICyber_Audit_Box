...
def test_817_missing_policy_blocks_compliant_even_with_valid_evidence():
    """8.17 Clock Synchronization: strong NTP evidence but NO policy -> NON_COMPLIANT.

    The dual rule (.agents/AGENTS.md): COMPLIANT requires POLICY_STATUS=FOUND,
    POLICY_ASSESSMENT=COMPLIANT, EVIDENCE_STATUS=FOUND and EVIDENCE_ASSESSMENT=COMPLIANT,
    with NO exceptions -- a missing policy is always a gap. This test previously
    asserted the opposite (evidence alone could pass, via a NOT_REQUIRED policy state
    that no longer exists in the schema).

    The evidence fields must stay honest while the policy gap drives the verdict:
    a real, direct, grounded NTP proof is still EVIDENCE_STATUS=FOUND /
    EVIDENCE_ASSESSMENT=COMPLIANT -- the missing policy is not a reason to
    retroactively downgrade good evidence.
    """
    finding = {
    ...
    # policy_required is still derived and persisted for reporting, but it no
    # longer gates the verdict -- the dual rule applies either way.
    assert result["policy_required"] is False
    assert result["policy_status"] == "NOT_FOUND"
    assert result["policy_assessment"] == "NON_COMPLIANT"
    assert result["evidence_status"] == "FOUND"          # good evidence stays good
    assert result["evidence_assessment"] == "COMPLIANT"
    assert result["final_result"] == "NON_COMPLIANT"     # ...but the policy gap decides
    assert result["status"] == "NON_COMPLIANT"
    assert result["policy_present"] == "Not Found"


def test_817_missing_policy_and_missing_evidence_non_compliant():
    """8.17 Clock Synchronization: no policy AND no operational evidence -> NON_COMPLIANT.

    The both-missing case, kept as the counterpart to the test above so the pair
    pins down that neither half alone carries a control.
    """
    finding = {
        "control_id": "8.17 Clock Synchronization",
        "control_name": "Clock Synchronization",
