...
class AuditReport(Base):
    ...
    # restart, produces the same document instead of silently reverting to the
...
class AuditCheckpoint(Base):
    ...
    # ── Run shape, so a resume continues the SAME audit ──────────────────────
    # None of this was stored, and /resume-checkpoint hardcoded "Deep" and passed
    # no custom evidence -- so resuming a Quick run silently switched it to Deep,
    # and resuming an Excel-scoped run dropped the locked_filenames entirely:
