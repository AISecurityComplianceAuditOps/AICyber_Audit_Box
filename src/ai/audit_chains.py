...
════════════════════════════════════════
TODAY'S DATE IS {current_date}. This is the real current date. Use ONLY this date for every
comparison involving time. Do NOT use your training data to guess what year or month it is --
your internal sense of "now" is out of date and WILL be wrong.
  * A date EARLIER than {current_date} is in the PAST. A policy or evidence artifact dated in the
    past is completely normal and is NOT by itself a defect -- do not call it "future-dated",
    do not claim it "has not taken effect yet", and do not question its validity on that basis.
  * A date is "in the future" ONLY if it falls strictly AFTER {current_date}.
  * Before writing that anything is future-dated, expired, overdue or stale, compare it against
    {current_date} explicitly and make sure the direction is right.

EVERY DATE CLAIM MUST BE DEFENSIBLE. If you state that a policy or evidence artifact is EXPIRED,
STALE, REVIEW_OVERDUE, out of date, or not yet in effect, you MUST be able to point at the proof:
  * Name the exact date the document itself states (e.g. "Expiry: 01 Jan 2026", "Next review:
    March 2026") or the document's own explicit wording ("superseded", "obsolete", "deprecated",
    "no longer valid", "retired", "end of life"), AND
  * State the comparison you made against {current_date}.
If the document states no expiry or review date and uses none of that wording, the correct answer
is UNKNOWN. Never infer that something is stale or expired from its age alone, from a missing date,
from a version number, or because a document simply "looks old" -- an auditor will ask what you
based it on, and "it was dated a while ago" is not an answer that survives that question.
...
FINAL_RESULT = COMPLIANT ONLY when ALL FOUR of these are true, with all mandatory atomic
requirements of the control/question affirmatively satisfied by grounded evidence:
  1. POLICY_STATUS = FOUND
  2. POLICY_ASSESSMENT = COMPLIANT
  3. EVIDENCE_STATUS = FOUND
  4. EVIDENCE_ASSESSMENT = COMPLIANT
There are NO exceptions. A missing policy is always a gap: if either the policy or the evidence
is missing (NOT_FOUND) or fails its assessment (NON_COMPLIANT), FINAL_RESULT = NON_COMPLIANT.
Do NOT emit NOT_REQUIRED or NOT_APPLICABLE for either field -- those values do not exist in this
schema, and the deterministic Python layer that re-computes this result does not recognise them.
...
<justification>ANSWER THE AUDIT CHECK QUESTION DIRECTLY — the ORIGINAL AUDIT CHECK QUESTION if one is stated above, otherwise the Control Name / requirement stated above — the way a knowledgeable colleague would answer it if asked out loud. Write it in this order:
(1) OPENING = the answer to the question exactly as it was asked, stated in YOUR OWN WORDS. Three shapes, depending on what was actually asked:
    - Yes/no question ("Whether NTP is enabled?"): open with "Yes," / "No," / "Partially," and then the fact that settles it, in plain language — e.g. "Yes — NTP is enabled and synchronized on the host." NOT "Yes — NTP is enabled and synchronised: timedatectl reports 'NTP enabled: yes' and 'System clock synchronized: yes'." Do NOT paste literal command output, config lines, or log lines into this sentence — the exact source text is already captured separately as the evidence quote and shown to the auditor on its own; typing it again here is duplication, not proof.
    - what/how/which/when, asking for ONE thing ("How is authentication done?", "Which IP version is used?", "Version and last updated date?"): answer with the RESOLVED VALUE itself — e.g. "Authentication is done via SAML SSO with MFA enforced for operators." or "IPv4." — not with a yes/no, and not with the raw line the value was read from.
    - asking for SEVERAL distinct things at once ("CPU, memory and disk utilization?", "list the ports and protocols allowed"): answer EACH item by name with its own resolved value(s), one short sentence per item, in the same order the question named them — e.g. "CPU utilization: 6.69%, 5.21%, 10.3% and 12% across the monitored instances. Memory utilization: 99%, 89.2% and 99.2%. Disk utilization: 33.7%, 30.1% and 27.7%." Do not blend the items into one vague sentence, and do not answer only the first item and drop the rest — every item the question named needs its own value, not just the easiest one to find.
(2) Then any FURTHER facts the first sentence did not already give, each traceable to the submitted document. If the first sentence already carried the only fact there is, stop — a second sentence that adds nothing is worse than no second sentence.
(3) Then HOW those facts satisfy or fail the requirement — ONLY where that is not already obvious from (1). On a single-requirement control the first sentence has already settled it; restating it in different words is not a second point.
(4) Then, if it is not fully satisfied, exactly what is missing.
NEVER state the same fact twice in one justification, in the same words or reworded — an auditor reading "NTP is enabled" in sentence one does not need "which confirms NTP is enabled" in sentence two. Do NOT describe the evidence document itself ("The evidence provided is a terminal session showing...", "The screenshot shows...", "This document contains...") anywhere in the justification: the source file, page and exact quote are already recorded in their own fields, so repeating them here is noise. State what is TRUE of the system, not what the document is. Do NOT open with a preamble, restated control text, or filler like "The evidence provided indicates that..." — lead with the answer itself. Do NOT dump unorganized raw OCR lines, directory listings, or raw PDF text blocks. Every factual claim must be strictly traceable to the submitted document, and if the document does not answer the question, say plainly that it does not rather than answering around it.</justification>
...
The SAME rule applies when the audit check question itself asks for several distinct VALUES
rather than several requirements (e.g. "CPU, memory and disk utilization", "list the ports and
protocols allowed", several instances/hosts each reporting their own number). One <evidence_item>
per value, and each <excerpt> is that value exactly as it is written in the source (e.g. an
excerpt of "6.69 %" for one instance's CPU tile, "5.21 %" for another's) — never a sentence you
compose that combines several values together, since a composed sentence combining scattered
numbers will not exist anywhere in the source document and cannot be verified as a quote. The
crisp, organized summary of all these values belongs in the justification field above, in your
own words — evidence_items exists to prove each individual number is real and traceable, not to
also read well as prose.
...
AFFIRMATIVE EVIDENCE SAFEGUARD:
Never infer compliance from retrieval, grounding, support_status, policy status, or document relevance alone. Final compliance requires affirmative support for the exact requirement_question. Negative evidence (disabled, inactive, failed, expired, missing, denied, error, not configured) must result in NON_COMPLIANT even when the evidence is perfectly grounded.
An absent policy document is ALWAYS a gap: POLICY_STATUS=NOT_FOUND prevents COMPLIANT even when the operational evidence fully answers the check question. Evidence alone is never sufficient. Still evaluate whether the evidence affirmatively answers the exact ORIGINAL AUDIT CHECK QUESTION, and report the evidence fields honestly -- a strong evidence result with a missing policy is EVIDENCE_STATUS=FOUND / EVIDENCE_ASSESSMENT=COMPLIANT with POLICY_STATUS=NOT_FOUND and FINAL_RESULT=NON_COMPLIANT, not a downgrade of the evidence fields.
...
POLICY_VALIDITY/EVIDENCE_FRESHNESS:
* TODAY'S DATE IS {current_date}. This is the real current date -- use ONLY it for every time comparison, and do NOT guess the current year/month from your training data, which is out of date and WILL be wrong.
* A date EARLIER than {current_date} is in the PAST. A policy or evidence artifact dated in the past is normal and NOT by itself a defect -- never call it "future-dated" or claim it "has not taken effect yet". A date is "in the future" ONLY if it falls strictly AFTER {current_date}. Check the direction against {current_date} before writing that anything is future-dated, expired, overdue or stale.
...
* EVERY DATE CLAIM MUST BE DEFENSIBLE. To call anything EXPIRED, STALE, REVIEW_OVERDUE, out of date or not yet in effect, you MUST name the exact date the document states (e.g. "Expiry: 01 Jan 2026") or its own explicit wording ("superseded", "obsolete", "deprecated", "no longer valid", "retired", "end of life"), AND state the comparison you made against {current_date}. If the document states no expiry/review date and uses none of that wording, answer UNKNOWN. Never infer staleness from age alone, a missing date, a version number, or a document merely looking old -- an auditor will ask what you based it on.
...
rather than several requirements (e.g. "CPU, memory and disk utilization", "list the ports and
protocols allowed", several instances/hosts each reporting their own number). One <evidence_item>
per value, and each <excerpt> is that value exactly as it is written in the pre-extracted
paragraphs (e.g. an excerpt of "6.69 %" for one instance's CPU tile, "5.21 %" for another's) —
never a sentence you compose that combines several values together, since a composed sentence
combining scattered numbers will not exist anywhere in the pre-extracted paragraphs and cannot be
verified as a quote. The crisp, organized summary of all these values belongs in the justification
field above, in your own words — evidence_items exists to prove each individual number is real
and traceable, not to also read well as prose.
...
* TODAY'S DATE IS {current_date}. Use ONLY this date for time comparisons; do NOT guess the current
  year/month from your training data, which is out of date and WILL be wrong. A date EARLIER than
  {current_date} is in the PAST and a past-dated policy is normal, NOT a defect -- challenge any
  draft that called a past date "future-dated" or "not yet in effect". A date is in the future ONLY
  if it falls strictly AFTER {current_date}.
* DEFENSIBILITY: challenge any EXPIRED/STALE/REVIEW_OVERDUE claim the draft cannot back with either
  an exact date stated in the document or the document's own explicit wording ("superseded",
  "obsolete", "deprecated", "no longer valid", "retired", "end of life"), plus the comparison
  against {current_date}. Staleness inferred from age alone, a missing date, or a version number is
  not defensible -- downgrade it to UNKNOWN.
...
class NativeOllamaChain:
    ...
    def invoke(self, input_dict: dict) -> AuditFindingSchema:
        ...
        # The model has no idea what today's date is -- it answers from its training
        # cutoff, which is in the past. Observed on a real finding: a policy dated
        # April 2026 was reported as "dated in the future (April 2026), which
        # invalidates its current status", months AFTER April 2026 had passed. The
        # deterministic layer (validator.resolve_policy_validity) compares against
        # datetime.now() and got it right, but the model's own free-text
        # business_impact/justification is what the auditor actually reads, and it
        # was contradicting both the structured verdict and reality. Supplying the
        # real date is the only thing that lets the model reason about "expired",
        # "future-dated", "overdue" or "within the review cycle" correctly.
        from datetime import datetime as _dt
        _now = _dt.now()
        input_dict = {
            **input_dict,
            "framework_name": framework_name,
            "other_frameworks_examples": other_frameworks_examples,
            "current_date": _now.strftime("%d %B %Y"),
        }
        ...
        try:
            ...
            if _stats.get("truncated"):
            ...
            elif any(k in err_msg.lower() for k in (
                ...
                # the wording used to blame the uploaded evidence for what is an
                # internal formatting failure.
                business_impact = (
                    f"Unable to automatically verify control {control_id_str}: the AI's response could not be "
                    "parsed into a valid finding. Manual verification is recommended."
                )
                justification = (
                    f"The model's response for {control_id_str} was not in the expected format and could not be "
                    "parsed, so no assessment was produced. This is a generation/formatting issue rather than a "
                    "conclusion about the uploaded document."
                )
                missing_requirements = [
                    f"Manual validation required for control {control_id_str} -- it was not successfully evaluated."
                ]
                recommendation = (
                    f"Re-run control {control_id_str}. If it keeps failing to parse, review the model's raw "
                    "output for this control before concluding anything about the evidence."
                )
