...
def generate_keywords(control_name, description="", expand_synonyms=True):
    """Auto-generate keyword signals from a control name + optional description.

    expand_synonyms=False keeps ONLY words the control itself uses. _SYNONYM_MAP is
    ISO-security-domain vocabulary, so expanding a control from another framework
    through it pulls in unrelated terms -- "SBOM Data Format Name" picked up
    "data classification", "data loss", "data masking" and "dlp" purely from the
    word "data", which would then let a DLP question match an SBOM format control.
    """
    ...
    # Bigrams are built from the SAME separator split as the tokens above. Using
    # combined.split() here instead left punctuation stranded as its own word, so a
    # control named "Lawful Basis / Consent Management" produced the keyword
    # fragments "basis /" and "/ consent".
