...
def _ensure_llama_server_running(port=11434):
    ...
    with lock:
        ...
        if port == 11434:
            ...
            # Match run_all.bat: one shared 128k KV pool rather than -np fixed
            # partitions. This used to request 262144 for a 12B model -- double
            # the launcher -- and omit --kv-unified, so each of the -np slots got
            # a fixed slice too small for the ~86k-token audit prompts.
            ctx_size = "131072"

            # ── Parallel slots (-np), sized against the model actually chosen ──
            # This block used to run BEFORE the model was selected, so it costed
            # a hardcoded 5.0 GB ("Gemma 4B Q4_K_M") while going on to load
            # gemma-4-12B-it-Q8_0 at 11.8 GB -- under-counting by ~7 GB and
            # over-provisioning slots on a box that could not feed them. It also
            # clamped to os.cpu_count(), which is the LOGICAL count (8 here), not
            # the physical cores its own comment claimed (4). Both pushed -np too
            # high, and the over-provisioned server then thrashed or died.
            try:
                import psutil as _psutil
                _avail_gb = _psutil.virtual_memory().available / (1024 ** 3)
                _model_gb = os.path.getsize(model_path) / (1024 ** 3)   # measured, not assumed
                _physical = _psutil.cpu_count(logical=False) or _physical_cores
                ...
                _np = max(1, min(_physical, int((_avail_gb - _overhead_gb) / _slot_gb)))
                print(f"[AUTO-START LLM] {_avail_gb:.1f}GB free RAM, model {_model_gb:.1f}GB, "
                      f"{_physical} physical cores -> -np {_np} (8-bit KV cache, 128k shared pool)",
                      flush=True)
            except Exception:
            ...
            cmd = [
                ...
                "--flash-attn", "on",
                "--cont-batching",
                # One shared KV pool instead of -np fixed partitions. Without it
                # each slot gets only ctx/np tokens -- 32k with -np 4 -- which is
                # far below the ~86k-token prompts a control actually sends, so
                # every audit request would be truncated or rejected.
                "--kv-unified",
                "-ctk", "q8_0",
                "-ctv", "q8_0",
            ]
