...
def validate_only(finding, document_text, expected_evidence_map, db_chunks=None):
    ...
    # str(): control_id arrives from the LLM, the DB and Excel rows, so it is not
    # guaranteed to be a string -- an int made .split() raise and killed the audit
    # for that control rather than degrading to an unresolved code.
...
def _parse_loose_date(date_str):
    ...
    # Month-year with no day ("April 2026", "Apr 2026", "2026-04", "04/2026").
    # Policies routinely state review/expiry at month granularity, and every one
    # of those returned None here -- so a policy whose document plainly says
    # "Expiry: March 2026" resolved to UNKNOWN instead of EXPIRED and the auditor
    # was never told it had lapsed. Resolved to the LAST moment of that month, not
    # the 1st: a policy that expires "April 2026" is still valid throughout April,
    # and anchoring to the 1st would report it expired up to a month early.
    import calendar
    month_year_formats = ["%B %Y", "%b %Y", "%Y-%m", "%m/%Y", "%m-%Y", "%Y/%m"]
    for fmt in month_year_formats:
        ...
        return parsed.replace(day=last_day, hour=23, minute=59, second=59)
    return None
...
# NOTE: _POLICY_DEMAND_MARKERS, _split_requirements, _drop_policy_demands,
# _rewrite_policy_recommendation and policy_required_reason were removed here.
#
# They existed to suppress "write a policy" wording when a control's policy was
# NOT_REQUIRED -- a state the schema no longer has. Under the current rule a policy
# is always required for a control-mapped finding, so "document a policy" is the
# CORRECT recommendation for a policy gap, and stripping it would have removed
# accurate auditor guidance. They had no callers in src/ (only their own tests), so
# they changed nothing in production either way -- but their 14 passing tests gave a
# false impression that this behaviour was still live and covered.
# policy_required_reason() went with them: the Excel-scoping waiver was its only
# caller, and that waiver was itself removed when the rule became unconditional.

_JUSTIFICATION_STOPWORDS = frozenset("""
    a an the this that these those it its is are was were be been being
    and or but which who whom whose what when where why how not no yes
    do does did has have had will would can could should shall may might
    must from at into onto than then there here also both each more most
    some such only own same very just so as of to for on in with by about
    over under again further once out up down off above below between
    through during before after because if while although though whether
    provided given based per per shown seen found stated noted per
""".split())

# Verbs whose object clause typically restates a claim already made -- "which
# confirms that X" says nothing X did not already say. Matched as the trigger
# for extracting the clause to compare, not for dropping the sentence outright.
_JUSTIFICATION_CONFIRM_RE = re.compile(
    r"\b(?:which\s+|and\s+|this\s+)?(?:explicitly\s+|clearly\s+|further\s+|also\s+)?"
    r"(?:confirms?|confirming|confirmed|verifies|verifying|verify|verified|proves?|proving|proved|"
    r"indicates?|indicating|indicated|demonstrates?|demonstrating|demonstrated|shows?|showing|showed|"
    r"validates?|validating|validated|reaffirms?|reaffirming|reaffirmed|reiterates?|reiterating|reiterated)"
    # "that" is REQUIRED, not optional. "showing"/"shows" are ordinary descriptive
    # verbs ("a session showing the output of...") that turn up constantly with
    # no restatement intent -- with "that" optional, .search() matched the FIRST
    # one of those instead of the real "...which explicitly confirms THAT NTP is
    # enabled..." further along, and swallowed everything in between as the
    # "clause", diluting the overlap check with unrelated document-description text.
    r"\b\s+that\s+(.*)$",
    re.IGNORECASE,
)
...
def _stem(word):
    """Just enough to fold plurals into their singular for matching purposes --
    "backups" vs "backup" should count as the same fact. Only ever compared
    against itself (both sides of every containment check run through this), so
    the exact stem does not need to be linguistically correct, only consistent.
    """
...
def _content_words(text):
    # No apostrophe in the character class: technical values are often quoted
    # ('NTP enabled: yes'), and keeping the apostrophe glued "'ntp" and "ntp"
    # apart as different tokens undercounted real overlap.
    words = re.findall(r"[a-z0-9]+", text.lower())
    return {_stem(w) for w in words if len(w) >= 2 and w not in _JUSTIFICATION_STOPWORDS}


def _dedupe_repeated_facts(text):
    """Drops a sentence that restates a fact an earlier sentence already gave.

    The generator prompt asks for the answer first, then supporting facts, then
    how those facts satisfy the requirement -- and on a single-requirement
    control, the last two collapse into the first, so the model often says the
    same thing twice: once plainly, once dressed up as "The evidence provided is
    a terminal session showing ..., which explicitly confirms that NTP is
    enabled and the system clock is synchronized." Word overlap on the whole
    sentence misses this (the document-description clause is full of NEW words
    -- "terminal", "session", "server" -- so a same-sentence-vs-prior-sentences
    comparison shows little overlap even though the CONFIRMED fact is a pure
    repeat). So a sentence built around a confirms/shows/indicates-that verb is
    judged on the clause AFTER that verb, not the sentence as a whole.

    Prompt engineering is the primary fix (see audit_chains.py) and works most
    of the time; this is the backstop for when it doesn't, so it stays
    deliberately conservative -- the first sentence is never touched, and a
    sentence is dropped only on clear, high-confidence overlap. Losing a
    genuinely new fact would be worse than leaving one redundant sentence in.
    """
    if not text or len(text) < 40:
        return text

    sentences = [x.strip() for x in _SENTENCE_SPLIT_RE.split(text.strip()) if x.strip()]
    if len(sentences) < 2:
        return text

    kept = [sentences[0]]
    seen = set(_content_words(sentences[0]))
    dropped = 0

    for sent in sentences[1:]:
        m = _JUSTIFICATION_CONFIRM_RE.search(sent)
        if m:
            clause_words = _content_words(m.group(1))
            if len(clause_words) >= 2:
                containment = len(clause_words & seen) / len(clause_words)
                if containment >= 0.5:
                    dropped += 1
                    continue
            kept.append(sent)
            seen |= _content_words(sent)
            continue

        # No confirms-style verb: only catch a near-verbatim repeat, at a
        # higher bar than rule A, since this compares the WHOLE sentence and a
        # sentence with no such verb has more surface area for genuinely new
        # words -- still below 0.8, which turned out to miss a plain restated
        # sentence differing only by a few trailing connector words.
        full_words = _content_words(sent)
        if len(full_words) >= 3:
            containment = len(full_words & seen) / len(full_words)
            if containment >= 0.7:
                dropped += 1
                continue
        kept.append(sent)
    ...
    print(f"[JUSTIFICATION DEDUP] Dropped {dropped} redundant sentence(s) "
          f"({len(sentences)} -> {len(kept)}).", flush=True)
    return " ".join(kept)


_QUOTED_SPAN_RE = re.compile(r"['\"]([^'\"]{4,})['\"]")

# A run of 3+ characters containing at least one digit -- an IP octet, a
# version number, a port, a date, a count. Used to catch a genuinely new value
# sitting in the same after-colon clause as a leftover quote: e.g. "server IP
# is 192.168.10.15, timedatectl reports 'NTP enabled: yes'." The compound
# question this defends is exactly "is X enabled, and what is the server IP" --
# a real audit question this app has to answer correctly, not just avoid
# breaking by luck.
_VALUE_TOKEN_RE = re.compile(r"[a-z0-9][a-z0-9.\-:/]{1,}[a-z0-9]|[0-9]")


def _has_unaccounted_value(after, quoted_spans, before):
    """True if `after`, once the matched quotes are lifted out, still contains
    a number-bearing token that is not already present in `before`. That token
    is assumed to be a fact the auditor would lose -- an IP, a version, a
    date, a count -- and blocks the trim.

    Checked against `before` ONLY -- not against the quoted content too. The
    quoted content is exactly what is about to be deleted, so it cannot be
    used to justify deleting something else: "banner shows 'OpenSSH_9.6',
    confirming SSH_version is 9.6." must not trim away "9.6" just because that
    digit also happens to appear inside the quote being removed -- once the
    quote is gone, "9.6" existed nowhere else, and the version number itself
    (as opposed to the raw banner string) IS the kind of resolved-value answer
    the generator prompt asks for and this pass must not undo.

    Erring toward "leave it" here is the safe direction: worst case a
    redundant quote survives, which is what this whole feature already
    tolerated before this pass existed.
    """
    residual = after
    for q in quoted_spans:
        residual = residual.replace("'" + q + "'", " ").replace('"' + q + '"', " ")
    covered = before.lower()
    for tok in _VALUE_TOKEN_RE.findall(residual.lower()):
        if any(ch.isdigit() for ch in tok) and tok not in covered:
            return True
    return False


def _trim_literal_evidence_quote(text, quote_sources):
    """Cuts a sentence off at the colon when everything after it is just the
    literal evidence text repeated inline -- e.g. "Yes -- NTP is enabled and
    synchronized: timedatectl reports 'NTP enabled: yes' and 'NTP synchronized:
    yes'." The auditor already sees that exact text verbatim in the Exact
    Evidence / Documented Policy panel on the same finding card -- quoting it
    again inside the narrative answer is not a second piece of information,
    it is the same one shown twice on the same page. The plain-language answer
    before the colon is the part worth keeping.

    Only fires when EVERY quoted fragment after the colon is a verbatim
    substring of the finding's OWN recorded evidence/policy quote -- a
    specific, checkable fact, not a guess about writing style -- when what
    remains before the colon still reads as a complete answer on its own (at
    least 3 content words, so a colon used for some other purpose is left
    alone rather than truncated into a fragment) -- and when nothing else in
    the after-colon clause looks like an unaccounted-for value. A compound
    question ("Is NTP enabled, and what is the server IP?") can legitimately
    put a real answer (the IP) in the same clause as a leftover quote, and
    that answer must not be discarded along with the quote.
    """
    if not text or ":" not in text or not quote_sources:
        return text
    ...
    for sent in sentences:
        if ":" not in sent:
            out.append(sent)
            continue
        before, _, after = sent.partition(":")
        before = before.strip()
        after = after.strip()
        quoted = _QUOTED_SPAN_RE.findall(after)
        if (quoted and len(_content_words(before)) >= 3
                and all(any(q.lower() in src.lower() for src in quote_sources) for q in quoted)
                and not _has_unaccounted_value(after, quoted, before)):
    ...
    if not trimmed:
        return text
    print(f"[JUSTIFICATION DEDUP] Trimmed {trimmed} clause(s) that just repeated "
          f"the recorded evidence quote inline.", flush=True)
    return " ".join(out)
...
def _clean_finding_narrative_fields(finding):
    ...
    quote_sources = [
        ...
        finding.get("operational_evidence_snippet"), finding.get("policy_snippet"),
    ]
...
def post_process(finding, document_text, expected_evidence_map=None, db_chunks=None):
    ...
    if finding.get("status") != "FALSE_POSITIVE":
        ...
        for idx, req_text in enumerate(raw_reqs):
            ...
            # NOTE: an Excel-scoping policy waiver used to sit here. It let a control the
            # auditor's checklist had locked to evidence-only files (e.g. 5.33 scoped to a
            # single screenshot, which cannot contain a policy) pass without one, on the
            # reasoning that failing it for a missing artifact the auditor never put in
            # scope was the tool overruling the auditor.
            #
            # It has been removed deliberately. The compliance rule is now unconditional
            # (.agents/AGENTS.md): COMPLIANT requires POLICY_STATUS=FOUND and
            # POLICY_ASSESSMENT=COMPLIANT alongside the evidence pair, with no exceptions,
            # so a scoped-to-a-screenshot control reports a genuine policy gap rather than
            # a pass. The waiver had in any case been inert since the 2-state change in
            # 139f6e4 -- it set policy_req=False and a policy_requirement_waived flag that
            # the final compliance formula no longer consults, so it silently stopped
            # affecting any verdict while still logging that it had "waived" something.
        ...
        #
        # The ONE documented exception is Customize (question-only) scope. There the
        # auditor's checklist question is the audit item and is deliberately not mapped
        # to any control or policy document, so there is no policy artifact to expect --
        # applying the dual rule would fail every row by construction and make the mode
        # useless. Such a row is judged on whether the evidence answers the question.
        # This is keyed off the finding actually being question-only, NOT off a missing
        # control_id, so a control-mapped finding that merely failed to resolve its id
        # can never slip into the evidence-only path.
        _customize_mode = bool(finding.get("customize_mode")) or \
            str(finding.get("scoping_mode") or "").upper() == "CUSTOMIZE"
        ...
        is_compliant = (
            (evidence_ok if _customize_mode else (policy_ok and evidence_ok))
        ...
        finding["policy_required"] = False if _customize_mode else policy_required
        ...
        # Customize (question-only) rows carry no policy dimension at all, so the policy
        # fields are cleared rather than left at NOT_FOUND. Leaving them populated made
        # the UI render a "⚠ Policy: Not Found" badge and the export print a policy gap
        # on a mode that never asked for a policy -- reporting a deficiency against a
        # requirement the auditor deliberately excluded. Blank fields make the frontend's
        # own `hasPolicyFields` check false, so no policy badge is drawn.
        if _customize_mode:
