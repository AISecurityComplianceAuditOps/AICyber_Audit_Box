...
def save_document_chunks(filename, text, report_id=None):
    ...
    try:
        with force_master():
            ...
            if cached_chunks:
                chunks_saved = 0
                # Same repeat-sentence problem as the fallback path below, milder here
                # because these parents come from doc_parsers' own structure rather than
                # from sliding windows -- measured at 4 repeats in 121 rows on a real
                # policy .docx. Skipped for the same reason: a duplicate row only costs
                # an embedding call and a slot in the fixed-size retrieval top-K.
                seen_child_content = set()
                for idx, (p_content, metadata) in enumerate(cached_chunks):
                    ...
                    for s_idx, sentence in enumerate(sentences):
                        ...
                        if child_content in seen_child_content:
                            continue
                        seen_child_content.add(child_content)
            ...
            stride = 1
            chunks_saved = 0
            # Overlapping windows (size 3, stride 1) mean each paragraph appears in up
            # to three consecutive windows, and every window is then split into child
            # sentences -- so the SAME sentence was stored as up to three separate rows.
            # Measured on a real 21K-char evidence document: 367 rows stored for 227
            # distinct sentences, i.e. 38% of rows (and 38% of the embedding calls they
            # each trigger) were exact repeats. Retrieval already discards them later
            # via exact-match and Jaccard dedup, so they only ever cost embedding time
            # and crowded out genuinely distinct candidates in the fixed-size top-K
            # window. Skipping them at write time keeps every distinct sentence, and the
            # first occurrence keeps its full 3-paragraph parent_context, so the
            # sentence-window expansion at retrieval time is unchanged.
            seen_child_content = set()
            for idx in range(0, len(section_aware_paras), stride):
                ...
                for s_idx, sentence in enumerate(sentences):
                    ...
                    # Same sentence already stored for this file from an earlier
                    # overlapping window -- see seen_child_content above. Compared on the
                    # final child_content (section prefix included), so the same sentence
                    # appearing under two DIFFERENT section headings is correctly kept as
                    # two distinct chunks rather than being collapsed into one.
                    if child_content in seen_child_content:
                        continue
                    seen_child_content.add(child_content)
...
def _retrieve_rag_context(
    ...
    if tokenized_corpus:
        ...
        if combined_scores is None:
            # Fallback: original plain term-count x weight scorer. The scoring loop
            # itself had been emptied out to a bare `score = 0`, so when BM25 was
            # unavailable every chunk tied at zero -- which silently degrades this
            # to vector-only ranking, or, with no query embedding either, to the
            # arbitrary first 20 chunks in DB order.
            combined_scores = []
        ...
        if combined_scores and chunk_texts:
            ...
            for score, t in zip(combined_scores, chunk_texts):
                ...
                # The boosted score has to be COLLECTED, not just computed. This loop
                # previously computed score_mult and discarded it, leaving
                # boosted_scores empty -- and the assignment below then replaced every
                # real BM25 score with that empty list. Every downstream stage reads
                # `zip(combined_scores or [], ...)`, so retrieval silently selected
                # zero chunks and fell through to the raw context[:6000] slice at the
                # end of this function: the same opening 6000 characters handed to
                # every control, no ranking, no reranking, no provenance. Measured on
                # a real run before the fix: 235 chunks indexed, 0 selected.
                score_mult = 1.0 + (gov_matches * 0.15) - (intro_matches * 0.10)
                boosted_scores.append(score * score_mult)
