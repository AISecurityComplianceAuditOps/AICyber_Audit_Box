...
def _resolve_llm_model(model_choice):
    ...
    Matched case-insensitively. The dropdown's actual value is "Gemma 4 (12b)"
    (lowercase b) while this used to check for "12B" (uppercase) -- so a 12B
    selection never matched, fell through to the generic "contains Gemma"
    branch, and got mislabeled "gemma2:9b" in logs and telemetry. Harmless to
    the actual generation (this string only ever picks a prompt template, and
    "gemma2:9b" still contains "gemma"), but confusing and wrong to record.
    """
    MODEL_MAP = {
    ...
    choice_upper = str(model_choice or "").upper()
    if "12B" in choice_upper:
        return "gemma4:12b"
    if "E4B" in choice_upper or "4B" in choice_upper:
        return "gemma4:e4b"
    if "GEMMA" in choice_upper or "9B" in choice_upper or "2B" in choice_upper:
        return "gemma2:9b"
    return "qwen2.5:7b"
        ...
        if excel_items and isinstance(excel_items, list):
            ...
            for item in excel_items:
                ...
                controls.append({
                    ...
                    # Customize scope: the control mapping above still applies (the
...
def _safe_json(value):
    """Serialize checkpoint side-data, never letting it break the checkpoint.

    A scoping payload that will not serialize is worth losing; the checkpoint
    that lets the auditor resume at all is not.
    """
    if value is None:
        return None
    try:
        ...
        print(f"[checkpoint] Could not serialize run shape ({e}); resume will use defaults.", flush=True)
        return None
...
def _checkpoint_update(session_id, completed_batches, all_results_so_far):
    ...
    with force_master():
        db = SessionLocal()
        try:
            # "interrupted" included for the same reason as in
            # _checkpoint_update_per_control below.
            chk = db.query(AuditCheckpoint).filter(
                AuditCheckpoint.session_id == session_id,
                AuditCheckpoint.status.in_(["in_progress", "paused", "interrupted"])
            ).first()
            if chk:
...
def _checkpoint_update_per_control(session_id, control_id, all_results_so_far):
    ...
    with force_master():
        db = SessionLocal()
        try:
            # "interrupted" belongs here too. A stop only takes effect at the top
            # of the next control, so the stopping thread can still be finishing
            # the one it was on when the auditor clicks Resume -- and it writes
            # "interrupted" as its last act. Without that status in this filter,
            # every checkpoint write from the resumed run silently found no row
            # and did nothing, so a second stop lost everything since the resume.
            chk = db.query(AuditCheckpoint).filter(
                AuditCheckpoint.session_id == session_id,
                AuditCheckpoint.status.in_(["in_progress", "paused", "interrupted"])
            ).first()
...
def _checkpoint_has_remaining(session_id):
    """True when this session's checkpoint still has controls left to evaluate.

    Guards the "interrupted" status below: a stop that lands after the final
    control already finished is not an interruption, and marking that checkpoint
    resumable would offer the auditor a Resume button with nothing to run.
    completed_control_ids_json is the reliable record -- it is written after
    EVERY control, including ones whose evaluation raised.
    """
    with force_master():
        db = SessionLocal()
        try:
            ...
            if not chk:
                return False
            try:
                done = len(json.loads(chk.completed_control_ids_json or "[]"))
            except Exception:
                done = chk.completed_controls or 0
            return done < (chk.total_controls or 0)
        except Exception as e:
            print(f"[checkpoint] Remaining-work check failed: {e}", flush=True)
            return False
        finally:
            db.close()
...
Return format: ["topic1", "topic2", ...]"""
    ...
    for idx, c in enumerate(controls):
        ...
        state_input = {
            ...
            # Customize (question-only) scope: no control mapping and no policy
...
def _run_ollama_bg(bg_key, files_data, selected_sls_copy, ai_model, session_id=None, audit_mode="Deep", custom_docs=None, custom_evidence=None, file_registry=None, already_done_ids=None, username=None, scoping_mode=None):
    ...
    try:
        ...
        # A stop is not a completion. get_resumable_checkpoint(),
        # /interrupted-checkpoints and /resume-checkpoint all decide what can be
        # resumed by filtering this status field, and "completed" appears in none
        # of their lists -- so a manually stopped audit recorded itself as
        # finished and every control it had not yet reached was unrecoverable.
        # The stop flags are still set at this point (the thread's finally clears
        # them just below), which is what distinguishes a stop from a clean end.
        _was_stopped = bool(_bg_stop_flags.get(bg_key) or _bg_stop_flags.get(_sid))
        if is_resource_paused:
            _finish_status = "paused"
        elif _was_stopped and _checkpoint_has_remaining(_sid):
            _finish_status = "interrupted"
        else:
            _finish_status = "completed"
        _checkpoint_finish(_sid, _finish_status)
    except Exception as e:
...
def _run_fast_technical_vapt_bg(bg_key, files_data, selected_sls, file_registry=None,
    ...
    try:
        ...
        # ── Hybrid parser+LLM: opt-in remediation enrichment ─────────────────
        # Every field above -- severity, CVE list, control mapping, evidence --
        # came straight from the deterministic parsers and is already correct;
        # this only rewrites the "remediation" and "remediation_actionable"
        ...
        if ai_recommendations and all_findings:
            ...
            # fixed text on every scan -- the same paragraph about the objective of
            # vulnerability assessment, and the same sentence telling the client to
            # "follow the guidelines suggested by OWASP, OSSTMM and NIST", whether the
            # scan found three informational notices or an unauthenticated RCE. That is
            # the first thing a client reads. ONE call per report (not per finding),
            # written from the deterministic counts/hosts/CVEs computed above -- no
            ...
            try:
                ...
                if _narrative:
                    ...
                    # Persisted as well as cached: _bg_store is in-memory, so without
                    # this the same report exported after a restart -- or simply a week
                    # later -- would silently fall back to the static boilerplate and
                    # not match the document the auditor already sent the client.
                    try:
                        with force_master():
                            _db_n = SessionLocal()
                            _rep_n = _db_n.query(AuditReport).filter(
                                AuditReport.session_id == bg_key).first()
                            if _rep_n:
                                _rep_n.report_narrative_json = json.dumps(_narrative)
                                _db_n.commit()
                            _db_n.close()
                    except Exception as _persist_err:
                        print(f"[VAPT/PQC] Executive summary generated but not persisted "
                              f"(kept in memory for this export): {_persist_err}", flush=True)
                    print(f"[VAPT/PQC] Executive summary generated "
                          f"({len(_narrative.get('overview',''))} chars).", flush=True)
            except Exception as _narr_err:
                # None/failure means the report keeps its standard wording, which is
                # complete on its own -- never blocks the scan.
                print(f"[VAPT/PQC] Executive summary not generated, report keeps "
                      f"the standard text: {_narr_err}", flush=True)

        # ── Scope filter: honour the auditor's control selection ────────────────
        # selected_sls was accepted by this function and then never read -- so
        # deselecting controls in a VAPT/PQC scan did nothing at all, and the report
        # always contained every finding the parsers produced. The UI offered a
        # choice it silently ignored.
        #
        # Applied AFTER parsing and enrichment, never before: the parsers must still
        # see every file, so the excluded count below is the true number of findings
        # that existed, not a guess. Deliberately fails OPEN -- an empty/absent
        # selection, or a selection that maps to nothing, keeps every finding rather
        # than emptying a security report by accident.
        _excluded_count = 0
        try:
            _sel = {int(s) for s in (selected_sls or []) if str(s).strip().isdigit()}
            if _sel and all_findings:
                from src.core.controls_data import USE_CASES
                # sl -> the control id prefix findings carry ("VAPT-4", "PQC-5")
                _sel_ids = set()
                for uc in USE_CASES:
                    if int(uc.get("sl", -1)) in _sel:
                        _sel_ids.add(str(uc.get("use_case", "")).split(" ")[0].upper())
                if _sel_ids:
                    _kept = [
                        f for f in all_findings
                        if str(f.get("control_id") or "").split(" ")[0].upper() in _sel_ids
                    ]
                    _excluded_count = len(all_findings) - len(_kept)
                    # Never let a filter empty the report outright -- that reads as
                    # "nothing was found" when the truth is "nothing matched your
                    # selection", which is a dangerous thing to hand an auditor.
                    if _kept:
                        all_findings = _kept
                        if _excluded_count:
                            print(f"[VAPT/PQC SCOPE] {_excluded_count} finding(s) outside the "
                                  f"selected controls were excluded from this report "
                                  f"({len(all_findings)} kept).", flush=True)
                    else:
                        print(f"[VAPT/PQC SCOPE] The control selection matched none of the "
                              f"{len(all_findings)} finding(s); keeping all rather than "
                              f"producing an empty report.", flush=True)
                        _excluded_count = 0
        except Exception as _scope_err:
            print(f"[VAPT/PQC SCOPE] Scope filter skipped, keeping all findings: {_scope_err}", flush=True)
            _excluded_count = 0
