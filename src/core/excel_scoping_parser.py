...
def _build_non_iso_keyword_entries():
    """Keyword-map entries for the non-ISO frameworks, derived from their own names.

    Every one of the 96 hand-written entries above targets an ISO control, so the
    fast keyword path only ever worked for ISO 27001. SOC 2, DPDP, XBOM, BCMS and
    NIST CSF questions fell straight through to name-matching and the embedding
    fallback -- which does resolve them, but is the slower and less certain route.

    Derived rather than hand-written because there are ~110 of them and their names
    are already descriptive ("XBOM-6 SBOM Timestamp", "CC6.1 Logical Access
    Security"). Synonym expansion is OFF: _SYNONYM_MAP is ISO-security vocabulary,
    and expanding through it made "SBOM Data Format Name" answer to "dlp" and
    "data loss".

    Cross-framework contamination is not a risk here -- _resolve_control_by_direct_map
    looks its match up in the caller's framework-filtered use_cases, so an entry for
    another framework simply resolves to nothing and falls through as before.
    """
    try:
        from src.ai.keyword_generator import generate_keywords
        from src.core.controls_data import USE_CASES as _ALL_UC
    except Exception:
        return []

    entries = []
    for uc in _ALL_UC:
        std = str(uc.get("standard") or "").strip().upper()
        if not std or std.startswith("ISO"):
            continue                      # ISO already has hand-tuned entries above
        use_case = str(uc.get("use_case") or "").strip()
        if not use_case:
            continue
        # Drop the leading short id ("XBOM-6", "CC6.1", "DPDP-3") -- ids are matched
        # by _resolve_control_by_id, and leaving them here would add numeric noise.
        label = re.sub(r'^[A-Za-z]*-?\d+(?:\.\d+)*\s*', '', use_case).strip()
        if not label:
            continue
        # MULTI-WORD PHRASES ONLY. Single tokens are far too generic: an entry for
        # the bare word "access" (6 chars) outranks the hand-written ISO entry "pam"
        # (3 chars) under longest-match-wins, so "Whether PAM user access evidence
        # available?" stopped resolving to 8.2 Privileged Access Rights and landed on
        # whichever framework's access control happened to be first. Phrases like
        # "logical access" or "sbom timestamp" carry the specificity that makes a
        # match trustworthy, and are long enough not to be beaten by an unrelated
        # generic word.
        kws = [k for k in generate_keywords(label, expand_synonyms=False) if " " in k]
        if kws:
            entries.append((kws, use_case))
    return entries


# Appended, never inserted: the ISO entries keep their position and their
# longest-match-wins ordering is unaffected.
_DIRECT_KEYWORD_CONTROL_MAP = _DIRECT_KEYWORD_CONTROL_MAP + _build_non_iso_keyword_entries()
...
def parse_excel_scoping_checklist(
    ...
    customize_mode=True is "Customize" scope mode. Control resolution still runs
    exactly as normal -- the auditor uploads a question-only sheet, each question is
    matched to the control it corresponds to, and those controls are shown ticked in
    the controls list and evaluated. What Customize drops is the POLICY half: the
    verdict rests on whether the cited evidence answers the question, so a row is not
    failed for a missing policy document the auditor never put in scope.
    """
    try:
        import openpyxl
    except ImportError:
    ...
    try:
        ...
        for row_idx, row in enumerate(rows[header_row_idx + 1:], start=header_row_idx + 2):
            ...
            # Control resolution runs for every mode, Customize included: the auditor
            # uploads a question-only sheet and expects to see which control each
            # question matched, ticked in the controls list and evaluated. Customize
            # changes only whether a POLICY document is required for the verdict.
            ...
            items.append({
                ...
                # Marks the row as Customize scope so the compliance rule downstream
                # judges it on evidence alone. The control mapping above is unaffected --
                # this only removes the policy requirement, it does not detach the row
                # from its control.
                "customize_mode":               bool(customize_mode),
                "policy_required":              not customize_mode,
            })
