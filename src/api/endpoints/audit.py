...
def _load_report_narrative(report):
    ...
    Persisted on the report row by the VAPT/PQC worker when the auditor opted into
    AI recommendations. Falls back to the in-memory store for a report written by a
    worker running right now whose row has not been updated yet. None means the
    exporters keep their standard wording, which is a complete report on its own --
    so a missing or malformed narrative can never blank a section.
    """
    try:
        raw = getattr(report, "report_narrative_json", None)
        if raw:
            data = json.loads(raw)
        ...
        pass
    try:
        from src.core.bg_state import _bg_store
        cached = (_bg_store.get("report_narrative") or {}).get(getattr(report, "session_id", None))
...
class StartAuditRequest(BaseModel):
    ...
    # Set by the client only after the auditor has read and accepted the
    # re-run warning. Absent, a re-run that would mix evidence sets is refused
    # with a 409 carrying the details the warning is built from.
    confirm_rerun: Optional[bool] = False
    # Hybrid parser+LLM opt-in, VAPT/PQC only. The deterministic parser output
    # -- severity, CVE list, control mapping -- is unaffected either way; this
    # only asks the LLM to rewrite the remediation text so it names the actual
    # service/config found instead of a canned one-line placeholder, and to write
    # the report's executive summary from what this scan actually found.
    #
    # ON by default: the canned text it replaces ("Investigate service
    # misconfiguration and apply vendor patches/hardening.", "follow the guidelines
    # suggested by OWASP, OSSTMM and NIST") is what customers reported as generic,
    # and an auditor should not have to know to opt in to get a usable report.
    # Turning it OFF still gives a pure-parser scan with no LLM call at all, which
...
def api_admin_all_reports(request: Request, limit: int = 200):
    """Every auditor's audit reports, for admin oversight.

    /sessions is deliberately scoped to one user (an admin can pass ?username= to
    look someone up, but has to know who to ask for). There was no way to see what
    the team as a whole has produced, or to pull a finished report without knowing
    its session id -- so an admin could authorise a download they had no way to
    find. The export endpoints already permit admins (_assert_session_access lets
    role == "admin" through), so this only supplies the listing that was missing;
    it grants no access that did not already exist.
    """
    auth_user = _require_auth(request)
    ...
    try:
        ...
        # One grouped count rather than a query per report -- this list is for an
        # admin dashboard and would otherwise fire N+1 queries on every open.
        from sqlalchemy import func
        counts = dict(
            db.query(Finding.report_id, func.count(Finding.id))
              .group_by(Finding.report_id)
              .all()
        )
        saved_counts = dict(
            db.query(Finding.report_id, func.count(Finding.id))
              .filter((Finding.is_saved_to_shakthi == True) | (Finding.human_verified == True))
              .group_by(Finding.report_id)
              .all()
        )
        ...
        for r in reports:
            out.append({
                ...
                "auditor": r.assigned_auditor_username or r.created_by or "unknown",
                "framework": r.framework or "ISO 27001",
                "status": r.status or "Draft",
                "created_at": r.created_at.isoformat() if r.created_at else None,
                "reviewed_at": r.reviewed_at.isoformat() if r.reviewed_at else None,
                "total_findings": int(counts.get(r.id, 0)),
                # What a download would actually contain: the export filters to
                # saved/verified findings, so a report with 0 here produces an
                # empty document however many drafts it holds.
                "committed_findings": int(saved_counts.get(r.id, 0)),
            })
        return {"success": True, "reports": out, "count": len(out)}
    except HTTPException:
        raise
    except Exception as e:
        print(f"[ADMIN ALL REPORTS ERROR] {e}", flush=True)
        raise HTTPException(status_code=500, detail="Failed to load audit reports.")
    finally:
        db.close()
...
@router.post("/import-auditee-evidence")
def api_import_auditee_evidence(req: ImportAuditeeEvidenceRequest, request: Request):
    """Copies selected evidence files from an auditee's submitted session into
    the calling auditor's own active workspace session, so they can actually
    be evaluated in an audit run.

    The frontend's "Add to Workspace" button (addAuditeeFilesToWorkspace() in
    app.js) has been calling this exact route for a while; it never existed
    here. Every click 404'd, was caught by the frontend's own try/catch, and
    surfaced only as a generic "Network error adding files to workspace" toast
    -- indistinguishable from an actual connectivity blip. An auditee's
    uploaded evidence could be BROWSED in the sidebar (GET /auditee-sessions +
    GET /evidence) but never actually pulled into an auditor's own session to
    be used for anything.

    Files are COPIED -- a new file on disk under the target report, a new
    EvidenceFile row -- rather than re-pointed at the source's own file_path.
    Sharing the physical file between two sessions would mean a delete in
    either one silently affects the other; every other evidence file in this
    app is independent once it belongs to a report, and an imported one is no
    different.
    """
    auth_user = _require_auth(request)
    if not req.filenames:
        raise HTTPException(status_code=400, detail="No files selected to import.")

    db = SessionLocal()
    try:
        with force_master():
            src_report = db.query(AuditReport).filter(AuditReport.session_id == req.source_session_id).first()
            if not src_report:
                raise HTTPException(status_code=404, detail="Source auditee session not found.")
            _assert_session_access(db, src_report, auth_user)

            tgt_report = db.query(AuditReport).filter(AuditReport.session_id == req.target_session_id).first()
            if not tgt_report:
                raise HTTPException(status_code=404, detail="Target workspace session not found.")
            _assert_session_access(db, tgt_report, auth_user)
            _assert_session_not_running(req.target_session_id, "add evidence")

            src_files = db.query(EvidenceFile).filter(
                EvidenceFile.report_id == src_report.id,
                EvidenceFile.filename.in_(req.filenames),
                (EvidenceFile.is_deleted == False) | (EvidenceFile.is_deleted.is_(None))
            ).all()
            if not src_files:
                raise HTTPException(status_code=404, detail="None of the selected files were found in the source session.")

            imported, skipped = [], []
            tgt_dir = os.path.normpath(os.path.join(os.getcwd(), "data", "evidence", str(tgt_report.id)))
            ...
            for sf in src_files:
                ...
                existing = db.query(EvidenceFile).filter(
                    EvidenceFile.report_id == tgt_report.id,
                    EvidenceFile.filename == sf.filename,
                    (EvidenceFile.is_deleted == False) | (EvidenceFile.is_deleted.is_(None))
                ).first()
                if existing:
                    skipped.append(sf.filename)   # already present in the workspace
                    continue

                # Same path-traversal guard as the regular upload endpoint.
                dest_path = os.path.join(tgt_dir, "auditee_import_" + sf.filename)
                if not os.path.abspath(dest_path).startswith(os.path.abspath(tgt_dir)):
                    skipped.append(sf.filename)
                    continue
                shutil.copyfile(sf.file_path, dest_path)

                db.add(EvidenceFile(
                    report_id=tgt_report.id,
                    filename=sf.filename,
                    file_path=os.path.abspath(dest_path),
                    is_auditor_uploaded=False,
                    status="Completed",
                ))
                imported.append(sf.filename)

            db.commit()
            try:
                log_system_event(
                    "AUDITEE_EVIDENCE_IMPORTED", "INFO",
                    f"Imported {len(imported)} file(s) from auditee session {req.source_session_id} "
                    f"into workspace {req.target_session_id}",
                    session_id=req.target_session_id, actor=auth_user.get("username"),
                )
            except Exception:
                pass
        ...
        db.rollback()
        print(f"[IMPORT AUDITEE EVIDENCE ERROR] {e}", flush=True)
        raise HTTPException(status_code=500, detail="Failed to import auditee evidence.")
    finally:
        db.close()
...
async def api_upload_evidence(
    ...
    try:
        with force_master():
            ...
            _assert_session_access(db, report, auth_user)
            # Uploading mid-scan is the mirror of deleting mid-scan: the file
            # lands, the auditor sees it in the list, and the running audit
            # never reads it -- because /audit/start already snapshotted the
            # evidence. The report then omits a file the screen says was
            # submitted, which is the same screen-vs-report contradiction as a
            # mid-run delete, only quieter.
            _assert_session_not_running(session_id, "add evidence")
            report_id = report.id
...
def _assert_session_not_running(session_id: str, action: str = "change this session's evidence"):
    """Raises 409 while an audit is running on this session.

    /audit/start snapshots everything the run needs -- the evidence bytes and
    `selected_sls_copy` are passed BY VALUE into the worker thread -- so a
    mid-run change cannot corrupt the run itself. The audit stays internally
    consistent whatever happens on screen.

    The damage is subtler than corruption, and worse for an audit product: the
    SCREEN AND THE REPORT COME TO DISAGREE. An auditor deletes an evidence file
    mid-scan, watches it disappear from the list, and the finished report still
    cites it -- because the running worker is holding bytes read before the
    delete. They believe they removed it. The report says otherwise, and
    nothing on either surface reveals the contradiction.

    The scope panel is already locked in the UI (lockScopeDisplay in app.js),
    but that is a disabled attribute, not a boundary: a direct API call walks
    straight past it. Same reasoning as clean_safe_text being the real
    validation boundary while escapeHtml() is the second layer.

    Both key shapes are checked because the codebase uses both -- /audit/start
    registers the raw session_id, while /audit/stop discards session_id AND
    "bg_{session_id}".
    """
    with _bg_lock:
        running = (session_id in _bg_running) or (f"bg_{session_id}" in _bg_running)
    if running:
        raise HTTPException(
            status_code=409,
            detail=(f"This audit is currently running, so you cannot {action}. "
                    f"The scan is using the evidence exactly as it was when it "
                    f"started. Stop the audit first, or wait for it to finish."),
        )


def _rerun_conflict(db, report):
    """Describe a re-run that would mix evidence sets, or None when it is safe.

    A fresh (non-resume) run purges unverified drafts but DELIBERATELY preserves
    findings the auditor human-verified or committed to the ledger -- correct on
    its own terms, since an auditor's accepted judgement should survive a
    re-scan.

    It stops being correct the moment the evidence changes underneath. Upload
    doc A, run, accept the findings, delete doc A, upload doc B, run again, and
    the session now holds accepted findings judged against a document that is no
    longer part of it, sitting in one report beside new findings judged against
    a different one. Nothing on the report distinguishes them.

    Detection compares the files those preserved findings CITE against the
    evidence currently attached. Reading it from source_files rather than a
    stored fingerprint means no new column and no migration -- and source_files
    is what the report itself prints, so if it is wrong the report was already
    wrong.

    Returns a dict the caller turns into a 409 body, so the warning the auditor
    reads is built from facts rather than a generic caution.
    """
    verified = db.query(Finding).filter(
        Finding.report_id == report.id,
        (Finding.human_verified == True) | (Finding.is_saved_to_shakthi == True),
    ).all()
    if not verified:
        return None                      # nothing preserved -- a re-run is clean

    cited = set()
    for f in verified:
        for name in str(f.source_files or "").split(","):
            name = name.strip()
            if name:
                cited.add(name)

    current = {
        e.filename for e in db.query(EvidenceFile).filter(
            EvidenceFile.report_id == report.id,
            (EvidenceFile.is_deleted == False) | (EvidenceFile.is_deleted.is_(None)),
        ).all() if e.filename
    }

    missing = sorted(cited - current)    # cited by an accepted finding, now gone
    added = sorted(current - cited)      # present now, no accepted finding cites it

    # Only the disappearance of cited evidence is a conflict. Added files alone
    # are the ordinary case of an auditor topping up evidence and re-running,
    # which is exactly what the tool is for.
    if not missing:
        return None

    return {
        "reason": "evidence_changed_since_verified_findings",
        "message": (
            f"This session already has {len(verified)} finding(s) you accepted or saved "
            f"to the ledger, and {len(missing)} of the file(s) they cite are no longer "
            f"attached. Re-running now would leave those findings in the report citing "
            f"evidence this session no longer holds."
        ),
        "verified_finding_count": len(verified),
        "missing_evidence": missing[:20],
        "added_evidence": added[:20],
        "session_status": report.status,
        "how_to_proceed": (
            "Re-run anyway to keep the accepted findings as they are, or clear them "
            "first if this session is being re-scoped to different evidence."
        ),
    }


def _resume_conflict(db, report, session_id, selected_sls=None):
    """Describe a resume that would finish the audit on different inputs, or None.

    A resume exists for the case where a run died halfway -- RAM pressure, a
    killed terminal, a machine restart -- and picking up at control 41 instead
    of redoing 1 to 40 is exactly right.

    It is only right while the inputs are the same. /audit/start re-reads the
    evidence list on EVERY start, resume included, so nothing carries the
    original snapshot forward. Between the crash and the resume an auditor can
    delete a document, and there is a window where they certainly can: _bg_running
    lives in memory, so a killed process leaves it empty and the running-audit
    lock that would normally refuse the delete is simply gone.

    The result is one audit report produced from two different evidence sets --
    controls 1 to 40 judged with the document, 41 to 91 judged without it, and
    nothing on the report saying so. Worse than the re-run case, because a
    re-run at least starts a visible new pass.

    AuditCheckpoint already records file_names_json and selected_sls_json, so
    the comparison needs no new column: it is checked against what the
    interrupted run was actually using, not against a guess.
    """
    ...
    with force_master():
        ...
        ).order_by(AuditCheckpoint.id.desc()).first()
    if chk is None:
        return None                      # nothing to resume; ordinary start rules apply
    ...
    then_files = {str(f).strip() for f in _load(chk.file_names_json) if str(f).strip()}
    if not then_files:
    ...
    removed = sorted(then_files - now_files)
    added = sorted(now_files - then_files)

    # A control-list change matters for the same reason and is recorded in the
    # same place, so it is reported alongside rather than in a separate guard.
    then_controls = {str(c) for c in _load(chk.selected_sls_json)}
    now_controls = {str(c) for c in (selected_sls or [])}
    controls_changed = bool(now_controls) and then_controls and (then_controls != now_controls)

    if not removed and not added and not controls_changed:
        return None

    parts = []
    if removed:
        parts.append(f"{len(removed)} evidence file(s) removed")
    if added:
        parts.append(f"{len(added)} added")
    if controls_changed:
        parts.append("the selected controls changed")

    return {
        "reason": "inputs_changed_since_interruption",
        "message": (
            f"This audit stopped after {chk.completed_controls or 0} of "
            f"{chk.total_controls or 0} control(s), and its inputs have changed since "
            f"({', '.join(parts)}). Resuming now would finish the report using a "
            f"different set of inputs from the one the completed controls were judged "
            f"against, with nothing on the report saying so."
        ),
        "completed_controls": chk.completed_controls or 0,
        "total_controls": chk.total_controls or 0,
        "removed_evidence": removed[:20],
        "added_evidence": added[:20],
        "controls_changed": controls_changed,
        "how_to_proceed": (
            "Restore the original evidence and resume, or start a fresh scan so every "
            "control is judged against the same inputs."
        ),
    }
    ...
    try:
        with force_master():
            ...
            if not report:
                raise HTTPException(status_code=404, detail="Session not found")
            _assert_session_access(db, report, auth_user)
            _assert_session_not_running(req.session_id, "delete evidence")
    ...
    try:
        with force_master():
            ...
            if not report:
                raise HTTPException(status_code=404, detail="Session not found")
            _assert_session_access(db, report, auth_user)
            _assert_session_not_running(req.session_id, "delete all evidence")
    ...
    try:
        with force_master():
            ...
            if not report:
                raise HTTPException(status_code=404, detail="Session not found")
            _assert_session_access(db, report, auth_user)
            _assert_session_not_running(req.session_id, "restore evidence")
            q = db.query(EvidenceFile).filter(EvidenceFile.report_id == report.id)
            if req.file_id:
                q = q.filter(EvidenceFile.id == req.file_id)
            elif req.filename:
...
def api_start_audit(req: StartAuditRequest, request: Request):
    ...
    try:
        with force_master():
            ...
            # Refuse a re-run that would leave accepted findings citing evidence
            # this session no longer holds -- unless the auditor has seen the
            # warning and said go ahead. Checked BEFORE the purge below, so a
            # refusal changes nothing.
            #
            # A resume is exempt: it continues the same run over the same
            # snapshot, which is the opposite of the situation this guards.
            if not req.confirm_rerun:
                if req.is_resume:
                    # A resume finishes an audit that is already half-judged, so
                    # the question is whether the second half will be judged on
                    # the same inputs as the first.
                    _conflict = _resume_conflict(db, report, req.session_id, req.selected_sls)
                else:
                    _conflict = _rerun_conflict(db, report)
                if _conflict:
...
def api_get_status(session_id: str, request: Request):
    ...
    if checkpoint:
        ...
        checkpoint_data = {
            ...
            # completed_batches only moves once every batch_size controls and is
            # never written by the per-control checkpoint path, so the UI banner
            # that keyed off it stayed hidden after a stop mid-batch. This is the
            # number the resume actually starts from.
            "completed_controls": checkpoint.completed_controls or 0,
...
# The ISO report template carries two distinct logo slots -- the auditee's (the
# audited organization, top of the cover page and the running header) and the
# auditor's own firm mark (header, and every non-ISO export). They are uploaded
# and reset independently; "auditor" is the default so the original single-logo
# callers keep working unchanged.
_LOGO_KINDS = {"auditor": "custom_company_logo.png", "auditee": "custom_auditee_logo.png"}


def _logo_asset_path(kind: str) -> str:
    filename = _LOGO_KINDS.get((kind or "auditor").strip().lower())
    if not filename:
        raise HTTPException(status_code=400, detail="Invalid logo kind. Allowed: auditor, auditee")
...
def api_upload_company_logo(request: Request, file: UploadFile = File(...), kind: str = "auditor"):
    """Saves a custom auditor or auditee logo for PDF/Word report exports."""
    user = _require_auth(request)
    # Auditors own the report, so they own its branding. This was admin-only while
    # the Report Metadata form that drives it is shown to auditors -- so an auditor
    # uploading a logo got a 403 they never saw (the preview is rendered locally by
    # FileReader and updates regardless), and the export silently kept the default
    # logo. Auditees are still excluded: they upload evidence, they do not brand
    # the auditor's report.
    if user.get("role") not in ("admin", "auditor"):
        raise HTTPException(status_code=403, detail="Access denied. Auditors and admins only.")
    try:
        ...
        logo_path = _logo_asset_path(kind)

        with open(logo_path, "wb") as f:
            f.write(raw_bytes)

        return {"success": True, "message": f"{kind.title()} logo uploaded successfully", "logo_path": logo_path}
    except HTTPException:
        raise
    except Exception as e:
...
def api_reset_company_logo(request: Request, kind: str = "auditor"):
    """Resets the auditor or auditee logo back to the default template logo."""
    user = _require_auth(request)
    # Matches the upload gate above -- an auditor who can set the logo must be able
    # to clear it, or the Reset button in the Report Metadata form is dead for them.
    if user.get("role") not in ("admin", "auditor"):
        raise HTTPException(status_code=403, detail="Access denied. Auditors and admins only.")
    try:
        logo_path = _logo_asset_path(kind)
        ...
        # An unknown logo kind is a 400 from _logo_asset_path -- without this it
        # would be swallowed and reported as a server error.
        raise
    except Exception:
...
_REPORT_BRANDING_FIELDS = (
    ...
    # Address/phone/mobile/report-date/order-reference: the DOCX master
    # template (report_exporter.py's _export_iso_template_docx) bakes in one
    # real past customer's real office address and phone numbers on its cover
    # page, and never substituted them -- every exported report shipped that
    # firm's real contact details regardless of which auditor name was set
    # above them. These let an auditor override that block the same way the
    # firm/auditor/document-ID fields already work.
    "brand_address", "brand_phone", "brand_mobile", "brand_report_date", "brand_order_ref",
    # The Report Metadata form has always had an Email box, but the frontend sent it
    # as "client_email" -- a name no endpoint declared, so FastAPI dropped it silently
    # and the value never reached the report.
    "brand_email",
    # "Dates of Audit" on the cover / "Audit dates" in the Details of Auditor
    # table -- previously stamped with the export date, which is not when the
    # audit actually ran.
    "brand_audit_dates",
    # The people the audit was conducted with ("Auditee Representatives" in
    # the report). Previously the auditee ORGANIZATION name was reused for
    # this row, so the report named a company where it should name people.
    "brand_auditee_reps",
)
...
def api_export_docx(
    ...
    brand_mobile: Optional[str] = None,
    brand_report_date: Optional[str] = None,
    ...
    _rb = _validate_report_branding(dict(
        ...
        brand_address=brand_address, brand_phone=brand_phone, brand_mobile=brand_mobile,
        brand_report_date=brand_report_date, brand_order_ref=brand_order_ref,
        brand_email=brand_email, brand_audit_dates=brand_audit_dates,
        brand_auditee_reps=brand_auditee_reps,
    ))
    (brand_firm, brand_auditor, brand_reviewer, brand_approver, brand_docid, brand_client,
     auditor_firm, auditor_lead, auditor_reviewer, auditor_approver, document_id, client_contact,
     brand_address, brand_phone, brand_mobile, brand_report_date, brand_order_ref,
     brand_email, brand_audit_dates, brand_auditee_reps) = (
        ...
        with force_master():
            ...
            for f in db_findings:
                ...
                findings_mapped.append({
                    ...
                    # Present only for question-based audits (Excel / Customize
                    # scoping). The exporters show it as the control point, since
                    # rows under one control differ only by their question -- it
                    # was never sent, so the reports could not show the question.
                    "requirement_question": f.requirement_question,
            ...
            meta_dict = {
                ...
                "brand_report_date": brand_report_date,
                "brand_order_ref": brand_order_ref,
                "brand_email": brand_email,
                "brand_audit_dates": brand_audit_dates,
                "brand_auditee_reps": brand_auditee_reps,
                ...
                "framework": report.framework or "ISO 27001",
                # Scan-specific executive summary / tactical recommendations, when the
...
- LEAD WITH THE ANSWER. First sentence answers the question directly. No preamble,
  no "Certainly", no restating the question back, no describing what you are about to do.
- BE SHORT. 2-4 sentences for a normal question. Use a short bullet list only when the
  answer really is a list (e.g. several findings, several steps); never more than 6 bullets.
- BE SPECIFIC TO THIS SESSION. When the question is about the audit, cite the actual
  control ids, findings and file names from the context above rather than describing
  ISO 27001 in general terms.
- SAY WHAT YOU DO NOT KNOW. If the context above does not contain the answer, say so in
  one sentence instead of filling the gap with generic compliance prose.
- NO FILLER. Drop "It is important to note", "As an AI", "In conclusion", and closing
  offers of further help."""

        assistant_reply = ""
        try:
            from src.core.llm_client import query_llm
            # timeout was 10s, which is shorter than a single generation takes on any
            # real deployment -- measured at ~21s here for a one-line question. So the
            # call raised ReadTimeout on essentially every message and the except below
            # swallowed it, silently serving the canned Knowledge Engine replies further
            # down instead of an actual answer. The assistant looked like it worked and
            # was never reaching the model at all. None lets llm_client apply the same
            # adaptive timeout every other call in the system uses.
            assistant_reply = query_llm(
                prompt_with_context,
                model=req.model_choice or "Gemma 4 (e4b)",
                timeout=None,
...
def _load_json_or_none(raw):
    """Checkpoint side-data that failed to store or parse must not block a resume."""
    if not raw:
        return None
    try:
...
def api_resume_checkpoint(req: dict, background_tasks: BackgroundTasks, request: Request):
    ...
    try:
        with force_master():
            ...
            # /audit/start uses the raw session_id as bg_key. This used to use
            # "bg_<session_id>", so every progress update and result the resumed
            # run wrote landed under a key /audit/status never reads: the poll saw
            # is_running False, found the report already marked "Pending Review"
            # by the stopped run, and reported "completed" while the resume was
            # still working through the remaining controls.
            bg_key = session_id
            ...
            # Without this the resumed run was invisible to every consumer of
            # _bg_running: /audit/status reported it idle, the concurrency caps
            # ignored it, and /audit/stop had nothing to discard. Clearing the
            # stop flags matters just as much -- /audit/stop sets them under both
            # the raw session_id and "bg_<session_id>", the worker thread pops
            # only its own key on exit, and nothing here cleared them, so the
            # resumed run hit the loop's stop check on its very first control and
            # exited having evaluated nothing.
            with _bg_lock:
                if bg_key in _bg_running:
                    return {"success": True, "status": "already_running",
                            "message": "This audit is already running.", "session_id": session_id}
                _bg_stop_flags.pop(bg_key, None)
                _bg_stop_flags.pop(f"bg_{session_id}", None)
                _bg_running.add(bg_key)
                _auditor_sessions[auth_user.get("username") or "auditor"].add(bg_key)
            ...
            # Rebuild the run the checkpoint recorded, not a default one. The
            # mode and the scoping payload were hardcoded to "Deep"/None here,
            # so a resumed Excel-scoped audit evaluated its remaining controls
            # against every uploaded file instead of the ones its checklist row
            # locked them to -- the second half of the audit was judged under
            # different rules from the first. Keyword arguments because
            # _run_ollama_bg takes custom_docs BEFORE custom_evidence, and the
            # positional form silently swapped them.
            _resume_mode = ck.audit_mode or "Deep"
            _resume_evidence = _load_json_or_none(ck.custom_evidence_json)
            _resume_docs = _load_json_or_none(ck.custom_docs_json)
        ...
        db.rollback()
        # The slot is reserved before the task is queued, so an error after that
        # point would otherwise leave the session marked running forever -- and
        # a session stuck in _bg_running can never be started or resumed again.
        with _bg_lock:
            _bg_running.discard(session_id)
            _auditor_sessions[auth_user.get("username") or "auditor"].discard(session_id)
